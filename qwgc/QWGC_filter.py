...
class QWGC_filter:
    '''
    This is another version of qwgc
    ...
    def optimize(self, train_data, train_label):
        ...
        new_data = np.array([j for i in n_amp for j in i])
        new_label = np.array(train_label*self.n_particle)
        ...
        personal_best_scores = [self._get_cost(new_data, new_label, theta)
        ...
        for t in trange(self.T, desc='training'):
            ...
            for n in range(self.n_particle):
                ...
                score = self._get_cost(new_data, new_label, particles[n])
            ...
            namp_data = np.array([self._zero_fill(amp, 2**theta_size)
                                  for amp in best_amp_data])
            error = self._get_cost(namp_data, train_label, grobal_best_pos)
            accs = self._get_accuracy(namp_data, train_label,
                                      grobal_best_pos)
            errors.append(error)
...
if __name__ == '__main__':
    ...
    qwgc = QWGC_filter(['01', '10'], Cp=p_pso['Cp'], Cg=p_pso['Cg'],
